# -*- coding: utf-8 -*-
"""optuna_rogers_satchell_optimizer_multi_zone.py
===============================================
- Optimise Rogers-Satchell pour différentes directions (SHORT/LONG) et types de volatilité (RANGE/EXTREM)
- Logique adaptative selon la stratégie :
  * SHORT + RANGE : Maximiser WR (entrer en SHORT en volatilité modérée)
  * SHORT + EXTREM : Minimiser WR (éviter SHORT en volatilité extrême)
  * LONG + RANGE   : Maximiser WR (entrer en LONG en volatilité modérée)
  * LONG + EXTREM  : Minimiser WR (éviter LONG en volatilité extrême)
...
import chardet
from Tools.func_features_preprocessing import calculate_rogers_satchell_numba
...
VOLATILITY_TYPE = "range"  # "range" ou "extrem"
...
if (DIRECTION == "Long" and VOLATILITY_TYPE == "range") or (DIRECTION == "Short" and VOLATILITY_TYPE == "range"):
    OPTIMIZATION_GOAL = "maximize"  # Maximiser le winrate (volatilité modérée favorable)
    GOAL_DESCRIPTION = "MAXIMISER"
else:
    OPTIMIZATION_GOAL = "minimize"  # Minimiser le winrate (éviter volatilité extrême)
    GOAL_DESCRIPTION = "MINIMISER"

print(
    f"{Fore.CYAN}🎯 STRATÉGIE: {DIRECTION} en volatilité {VOLATILITY_TYPE.upper()} → {GOAL_DESCRIPTION} le winrate{Style.RESET_ALL}")
...
# 🔧 PARAMÈTRES D'OPTIMISATION ADAPTATIFS POUR ROGERS-SATCHELL
if VOLATILITY_TYPE == "range":
    # Volatilité modérée (range) : chercher fenêtre étroite de volatilité optimale
    PERIOD_MIN, PERIOD_MAX = 5, 70
    # Basé sur vos résultats : 0.0001947 à 0.0001994
    LOW_THRESHOLD_MIN, LOW_THRESHOLD_MAX = 0.0001259, 0.0003939
    HIGH_THRESHOLD_MIN, HIGH_THRESHOLD_MAX = 0.0001259, 0.0003939
    THRESHOLD_STEP = 0.000001  # Très fin pour la précision
    TYPE_DESCRIPTION = "modérée (fenêtre étroite de volatilité optimale)"
elif VOLATILITY_TYPE == "extrem":
    # Volatilité extrême : chercher seuils pour détecter conditions défavorables
    PERIOD_MIN, PERIOD_MAX = 8, 25
    # Basé sur vos résultats : 0.0001559 à 0.0003939
    LOW_THRESHOLD_MIN, LOW_THRESHOLD_MAX = 0.0001259, 0.0003939
    HIGH_THRESHOLD_MIN, HIGH_THRESHOLD_MAX = 0.0001259, 0.0003939
    THRESHOLD_STEP = 0.0000001
    TYPE_DESCRIPTION = "extrême (conditions défavorables à éviter)"

print(f"📊 Volatilité {VOLATILITY_TYPE}: {TYPE_DESCRIPTION}")
print(f"🔧 Low threshold range: [{LOW_THRESHOLD_MIN:.6f}, {LOW_THRESHOLD_MAX:.6f}]")
print(f"🔧 High threshold range: [{HIGH_THRESHOLD_MIN:.6f}, {HIGH_THRESHOLD_MAX:.6f}]")

# 🔧 CRITÈRES DE VALIDATION ADAPTATIFS
if OPTIMIZATION_GOAL == "maximize":
    # Pour maximiser : critères standards
    WINRATE_MIN = 0.53
    # WR minimum acceptable
    PCT_TRADE_MIN = 0.06  # % de candles tradées minimum
    COMPARISON_OPERATOR = "≥"
else:
    # Pour minimiser : critères inversés
    WINRATE_MAX = 0.49  # WR maximum acceptable
...
LAMBDA_WR = 0.35  # Pénalité pour l'écart de WR entre datasets
LAMBDA_PCT = 0.45  # Pénalité pour l'écart de PCT entre datasets
...
param_summary = (
    ...
    f"▪️ Type volatilité : {VOLATILITY_TYPE} ({TYPE_DESCRIPTION})\n"
    f"▪️ Objectif : {GOAL_DESCRIPTION} le winrate\n"
    f"▪️ Period range : [{PERIOD_MIN}, {PERIOD_MAX}]\n"
    f"▪️ Low threshold range : [{LOW_THRESHOLD_MIN:.6f}, {LOW_THRESHOLD_MAX:.6f}]\n"
    f"▪️ High threshold range : [{HIGH_THRESHOLD_MIN:.6f}, {HIGH_THRESHOLD_MAX:.6f}]\n"
    f"▪️ Threshold step : {THRESHOLD_STEP}\n"
...
# ═══════════════════════ CALCUL ROGERS-SATCHELL AVEC LOGIQUE ADAPTATIVE ═══════════════════════

def calculate_rs_signal_adaptive(df_complete: pd.DataFrame, df_filtered: pd.DataFrame,
                                 period: int, low_threshold: float, high_threshold: float,
                                 direction: str, volatility_type: str) -> pd.Series:
    """
    🔧 Calcule Rogers-Satchell sur données COMPLÈTES avec logique adaptative selon direction/type
    ...
    Args:
        ...
        period: Période pour Rogers-Satchell
        low_threshold: Seuil bas de volatilité
        high_threshold: Seuil haut de volatilité
        direction: "Short" ou "Long"
        volatility_type: "range" ou "extrem"

    Returns:
        signal: Serie avec signal (0/1) indexée sur df_filtered
    """

    # 1) Rogers-Satchell sur toute la chronologie
    rs_complete = calculate_rogers_satchell_numba(
        df_complete["sc_high"].to_numpy(),
        df_complete["sc_low"].to_numpy(),
        df_complete["sc_open"].to_numpy(),
        df_complete["sc_close"].to_numpy(),
        session_starts=(df_complete["sc_sessionStartEnd"] == 10).to_numpy(),
        window=period,
    )

    # 2) Affectation + récupération par index (aucun merge, aucun risque d'erreur)
    df_complete["rs_value"] = rs_complete
    rs_filtered = df_complete.loc[df_filtered.index, "rs_value"].to_numpy()

    # Gestion des NaN éventuels
    rs_filtered = np.where(np.isnan(rs_filtered), 0.0002, rs_filtered)

    # 3) Construction du signal selon la logique adaptative
    in_band = (rs_filtered >= low_threshold) & (rs_filtered <= high_threshold)

    if direction == "Short":
        signal = ~in_band if volatility_type == "extrem" else in_band
        signal_description = f"SHORT en VOLATILITÉ {'EXTRÊME' if volatility_type == 'extrem' else 'MODÉRÉE'}"
    else:  # Long
        signal = ~in_band if volatility_type == "extrem" else in_band
        signal_description = f"LONG en VOLATILITÉ {'EXTRÊME' if volatility_type == 'extrem' else 'MODÉRÉE'}"

    # Debug : afficher la logique utilisée (seulement au premier appel)
    if not hasattr(calculate_rs_signal_adaptive, '_debug_printed'):
        range_desc = f"RS ∉ [{low_threshold:.6f}, {high_threshold:.6f}]" if volatility_type == "extrem" else f"RS ∈ [{low_threshold:.6f}, {high_threshold:.6f}]"
        print(f"🔧 Logique signal: {signal_description} ({range_desc})")
        calculate_rs_signal_adaptive._debug_printed = True

    return pd.Series(signal.astype(int), index=df_filtered.index)
...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    # 1️⃣ On tire d'abord la borne basse dans l'intervalle autorisé
    low_threshold = trial.suggest_float(
        "low_threshold",
        LOW_THRESHOLD_MIN,
        LOW_THRESHOLD_MAX,
        step=THRESHOLD_STEP,  # facultatif : retire-le si tu préfères un espace continu
    )

    # 2️⃣ Puis on tire la borne haute dans [low_threshold, HIGH_THRESHOLD_MAX]
    high_threshold = trial.suggest_float(
        "high_threshold",
        low_threshold,  # borne inf. = low_threshold ⇒ garantit l'ordre
        HIGH_THRESHOLD_MAX,
        step=THRESHOLD_STEP,
    )
    params = {
    ...
    signal_train = calculate_rs_signal_adaptive(TRAIN_COMPLETE, TRAIN_FILTERED,
                                                period, low_threshold, high_threshold, DIRECTION, VOLATILITY_TYPE)
    ...
    signal_val = calculate_rs_signal_adaptive(VAL_COMPLETE, VAL_FILTERED,
                                              period, low_threshold, high_threshold, DIRECTION, VOLATILITY_TYPE)
    signal_val1 = calculate_rs_signal_adaptive(VAL1_COMPLETE, VAL1_FILTERED,
                                               period, low_threshold, high_threshold, DIRECTION, VOLATILITY_TYPE)
    ...
    if USE_TRAIN_IN_OPTIMIZATION:
        print(f"Trial {trial.number:>5d} | P={period:>2d} Low={low_threshold:.6f} High={high_threshold:.6f} | "
...
def calculate_test_metrics_adaptive(params: dict):
    ...
    def calculate_dataset_stats(dataset_name, df_complete, df_filtered):
        ...
        signal = calculate_rs_signal_adaptive(df_complete, df_filtered,
                                              params['period'], params['low_threshold'], params['high_threshold'],
                                              DIRECTION, VOLATILITY_TYPE)
        mask = signal == 1
    ...
    signal_test = calculate_rs_signal_adaptive(TEST_COMPLETE, TEST_FILTERED,
                                               period, low_threshold, high_threshold, DIRECTION, VOLATILITY_TYPE)
    ...
    # Affichage
    print(
        f"Rogers-Satchell {DIRECTION} {VOLATILITY_TYPE.upper()} (Period={period}, Low={low_threshold:.6f}, High={high_threshold:.6f})")
...
def main():
    ...
    for done in range(1, N_TRIALS + 1):
        ...
        if done % PRINT_EVERY == 0:
            print(
                ...
                f"({DIRECTION.upper()} · {VOLATILITY_TYPE.upper()}) "
            ...
            if best_trial["number"] is not None:
                print(
                    f"\n{Fore.YELLOW}*** BEST so far for {DIRECTION} {VOLATILITY_TYPE} with {param_summary} ▸ trial {best_trial['number']}  score={Fore.GREEN}{best_trial['score']:.4f}{Style.RESET_ALL}")

                params = best_trial["params"]
                print(f"Meilleur: Trial #{best_trial['number']} | "
                      f"Period={params['period']} Low={params['low_threshold']:.6f} High={params['high_threshold']:.6f} | "
                ...
                print(
                    f"\n{Fore.YELLOW}*** BEST so far for {DIRECTION} {VOLATILITY_TYPE} with {param_summary} ▸ trial None  score=-inf{Style.RESET_ALL}")

    # ═══════════════════ SYNTHÈSE FINALE COMPLÈTE ═══════════════════
    print(f"\n{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}🏆 SYNTHÈSE FINALE - MEILLEUR RÉSULTAT ROGERS-SATCHELL OPTIMIZER{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")

    if best_trial.get("params"):
        bt = best_trial
        print(
            f"\n{Fore.YELLOW}*** BEST FINAL for {DIRECTION} {VOLATILITY_TYPE} with {param_summary} ▸ trial {bt['number']}  score={Fore.GREEN}{bt['score']:.4f}{Style.RESET_ALL}")
        print(
            f"    {Fore.CYAN}[Direction: {DIRECTION} - Volatilité: {VOLATILITY_TYPE.upper()} - Objectif: {GOAL_DESCRIPTION} WR]{Style.RESET_ALL}")
        ...
        print(f"    • Stratégie: {DIRECTION} en volatilité {VOLATILITY_TYPE.upper()} → {GOAL_DESCRIPTION} WR")
        print(f"    • Type volatilité: {TYPE_DESCRIPTION}")
        ...
        print(f"    • Low threshold: {params['low_threshold']:.6f}")
        print(f"    • High threshold: {params['high_threshold']:.6f}")
        print(f"    • Largeur de bande: {params['high_threshold'] - params['low_threshold']:.6f}")

        # Logique de signal Rogers-Satchell
        if VOLATILITY_TYPE == "range":
            signal_logic = f"Signal = 1 si RS ∈ [{params['low_threshold']:.6f}, {params['high_threshold']:.6f}] (volatilité dans la bande optimale)"
        else:  # extrem
            signal_logic = f"Signal = 1 si RS ∉ [{params['low_threshold']:.6f}, {params['high_threshold']:.6f}] (éviter volatilité extrême)"
    ...
    print(f"\n{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}🏁 FIN DE L'OPTIMISATION ROGERS-SATCHELL{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
