# -*- coding: utf-8 -*-
"""optuna_williams_r_optimizer_multi_zone.py
==========================================
- Optimise Williams %R pour différentes directions (SHORT/LONG) et zones (OVERBOUGHT/OVERSOLD)
- Logique adaptative selon la stratégie :
  * SHORT + OVERBOUGHT : Maximiser WR (entrer en SHORT en surachat)
  * SHORT + OVERSOLD   : Minimiser WR (éviter SHORT en survente)
  * LONG + OVERSOLD    : Maximiser WR (entrer en LONG en survente)
  * LONG + OVERBOUGHT  : Minimiser WR (éviter LONG en surachat)
...
import chardet
from Tools.func_features_preprocessing import compute_wr
...
ZONE = "oversold"  # "overbought" ou "oversold"
...
if ZONE == "overbought":
    # Zone surachat : Williams %R proche de 0 (ex: -20 à -2)
    PERIOD_MIN, PERIOD_MAX = 10, 80
    THRESHOLD_MIN, THRESHOLD_MAX = -20, -2
    THRESHOLD_STEP = 1
    ZONE_DESCRIPTION = "surachat (Williams %R proche de 0)"
elif ZONE == "oversold":
    # Zone survente : Williams %R proche de -100 (ex: -98 à -80)
    PERIOD_MIN, PERIOD_MAX = 10, 100
    THRESHOLD_MIN, THRESHOLD_MAX = -99, -70
    THRESHOLD_STEP = 1
    ZONE_DESCRIPTION = "survente (Williams %R proche de -100)"
...
if OPTIMIZATION_GOAL == "maximize":
    ...
    PCT_TRADE_MIN = 0.025  # % de candles tradées minimum
    COMPARISON_OPERATOR = "≥"
else:
    # Pour minimiser : critères inversés
    WINRATE_MAX = 0.49  # WR maximum acceptable (< 50%)
...
N_TRIALS = 10000
PRINT_EVERY = 25
ALPHA = 0  # Poids du WR dans le score
...
def load_csv_complete(path: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    ...
    Returns:
        df_complete: DataFrame avec TOUTES les bougies chronologiques (pour Williams %R)
...
# ═══════════════════════ CALCUL WILLIAMS %R AVEC LOGIQUE ADAPTATIVE ═══════════════════════

def calculate_williams_r_signal_adaptive(df_complete: pd.DataFrame, df_filtered: pd.DataFrame,
                                         period: int, threshold: float, direction: str, zone: str) -> pd.Series:
    """
    🔧 Calcule Williams %R sur données COMPLÈTES avec logique adaptative selon direction/zone
    ...
    Args:
        ...
        period: Période pour Williams %R
    ...
    # 1) Calculer Williams %R sur données COMPLÈTES (chronologiques)
    ...
    # Williams %R calculé sur TOUTE la chronologie
    will_r_complete = compute_wr(sc_high.values, sc_low.values, sc_close.values,
                                 session_starts=session_starts, period=period, fill_value=-50)

    # 2) ✅ APPROCHE PAR INDICES (plus rapide et fiable)
    df_complete['williams_r'] = will_r_complete
    williams_r_filtered = df_complete.loc[df_filtered.index, 'williams_r'].to_numpy()

    # Gestion des NaN éventuels
    williams_r_filtered = np.where(np.isnan(williams_r_filtered), -50, williams_r_filtered)

    # 3) 🔧 LOGIQUE ADAPTATIVE pour créer le signal
    if direction == "Short" and zone == "overbought":
        # SHORT en surachat : signal = 1 quand Williams %R >= threshold (proche de 0)
        signal_condition = williams_r_filtered >= threshold
        signal_description = f"SHORT en SURACHAT (WR >= {threshold})"

    elif direction == "Short" and zone == "oversold":
        # SHORT en survente : signal = 1 quand Williams %R <= threshold (proche de -100)
        signal_condition = williams_r_filtered <= threshold
        signal_description = f"SHORT en SURVENTE (WR <= {threshold})"

    elif direction == "Long" and zone == "oversold":
        # LONG en survente : signal = 1 quand Williams %R <= threshold (proche de -100)
        signal_condition = williams_r_filtered <= threshold
        signal_description = f"LONG en SURVENTE (WR <= {threshold})"

    elif direction == "Long" and zone == "overbought":
        # LONG en surachat : signal = 1 quand Williams %R >= threshold (proche de 0)
        signal_condition = williams_r_filtered >= threshold
        signal_description = f"LONG en SURACHAT (WR >= {threshold})"

    else:
        raise ValueError(f"Combinaison non supportée: direction={direction}, zone={zone}")

    # Debug : afficher la logique utilisée (seulement au premier appel)
    if not hasattr(calculate_williams_r_signal_adaptive, '_debug_printed'):
        print(f"🔧 Logique signal: {signal_description}")
        calculate_williams_r_signal_adaptive._debug_printed = True
...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    signal_train = calculate_williams_r_signal_adaptive(TRAIN_COMPLETE, TRAIN_FILTERED,
    ...
    signal_val = calculate_williams_r_signal_adaptive(VAL_COMPLETE, VAL_FILTERED,
                                                      period, threshold, DIRECTION, ZONE)
    signal_val1 = calculate_williams_r_signal_adaptive(VAL1_COMPLETE, VAL1_FILTERED,
                                                       period, threshold, DIRECTION, ZONE)
...
def calculate_test_metrics_adaptive(params: dict):
    ...
    def calculate_dataset_stats(dataset_name, df_complete, df_filtered):
        ...
        signal = calculate_williams_r_signal_adaptive(df_complete, df_filtered,
    ...
    signal_test = calculate_williams_r_signal_adaptive(TEST_COMPLETE, TEST_FILTERED,
                                                       period, threshold, DIRECTION, ZONE)
    ...
    # Affichage
    print(f"Williams %R {DIRECTION} {ZONE.upper()} (Period={period}, Threshold={threshold:.1f})")
...
def main():
    ...
    print(f"{Fore.CYAN}🏆 SYNTHÈSE FINALE - MEILLEUR RÉSULTAT WILLIAMS %R OPTIMIZER{Style.RESET_ALL}")
    ...
    if best_trial.get("params"):
        ...
        # Logique de signal Williams %R
        if DIRECTION == "Short" and ZONE == "overbought":
            signal_logic = f"Signal = 1 si Williams %R >= {params['threshold']:.1f} (SHORT en surachat)"
        elif DIRECTION == "Short" and ZONE == "oversold":
            signal_logic = f"Signal = 1 si Williams %R <= {params['threshold']:.1f} (SHORT en survente)"
        elif DIRECTION == "Long" and ZONE == "oversold":
            signal_logic = f"Signal = 1 si Williams %R <= {params['threshold']:.1f} (LONG en survente)"
        elif DIRECTION == "Long" and ZONE == "overbought":
            signal_logic = f"Signal = 1 si Williams %R >= {params['threshold']:.1f} (LONG en surachat)"
    ...
    print(f"\n{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}🏁 FIN DE L'OPTIMISATION WILLIAMS %R{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}{'=' * 80}{Style.RESET_ALL}")
