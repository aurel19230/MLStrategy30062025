...
def calculate_slopes_with_your_function(durations, volumes, session_starts, n_periods):
    """
    Utilise VOTRE fonction calculate_slopes_and_r2_numba optimisée
    SANS FALLBACK - Erreur claire si problème
    """
    try:
        print(f"🔧 Appel à calculate_slopes_and_r2_numba avec:")
        print(f"   - durations: {type(durations)} shape={durations.shape}")
        print(f"   - volumes: {type(volumes)} shape={volumes.shape}")
        print(f"   - session_starts: {type(session_starts)} shape={session_starts.shape}")
        ...
        # Appel à votre fonction optimisée
        ...
        print(f"✅ calculate_slopes_and_r2_numba réussie:")
        print(f"   - duration_slopes: {type(duration_slopes)} shape={duration_slopes.shape}")
        print(f"   - volume_slopes: {type(volume_slopes)} shape={volume_slopes.shape}")
        ...
        # Informations sur votre fonction
        print(f"\n🔍 Debug de votre fonction:")
        print(f"   Module: {calculate_slopes_and_r2_numba.__module__}")
        print(
            f"   Signature: {calculate_slopes_and_r2_numba.__annotations__ if hasattr(calculate_slopes_and_r2_numba, '__annotations__') else 'N/A'}")
...
class FastDataset:
    ...
    def calculate_slopes_fast(self, n_periods):
        ...
        # Utiliser VOTRE fonction optimisée sur toutes les données
        vol_slopes_complete, dur_slopes_complete, r2_vol, r2_dur = calculate_slopes_with_your_function(
